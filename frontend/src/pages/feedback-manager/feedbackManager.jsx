...
function FeedbackManager() {
  ...
  const loadFeedbacks = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(res.data) ? res.data.map(normalizeFeedback) : [];
      setFeedbacks(list);
    } catch (err) {
      setFeedbacks([]);
  ...
  const categories = useMemo(
    () => [...new Set(feedbacks.map((f) => f.item).filter((v) => v && v !== "—"))].sort(),
    [feedbacks]
  );
            ...
            <thead>
              <tr>
                ...
                <th className="sortable" onClick={() => changeSort("rating")}>
                  Đánh giá <span className="sort-ind">{sortIcon("rating")}</span>
                </th>
                <th>Nội dung</th>
                <th>Hình ảnh</th>
              </tr>
